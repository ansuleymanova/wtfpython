...
<!-- Generated using "markdown-toc README.md"-->

<!-- toc -->

- [Структура примеров](#Структура-примеров)
    + [▶ Some fancy Title](#-some-fancy-title)
- [Как пользоваться этим гайдом](#Как-пользоваться-этим-гайдом)
- [👀 Примеры](#-примеры)
  * [Раздел: Напряги мозги!](#раздел-напряги-мозги)
	+ [▶ Первым делом *](#-первым-делом-)
	+ [▶ Строки бывают обманчивы](#-строки-бывают-обманчивы)
    + [▶ Осторожнее с последовательными операциями](#-oсторожнее-с-последовательными-операциями)
	+ [▶ Как не нужно использовать `is`](#-как-не-нужно-использовать-is)
    + [▶ Хашбрауни](#-хашбрауни)
...
- Синтакс моржового оператора имеет форму `NAME:= expr`, где `NAME` - это валидный identifier, а `expr` - валидное выражение. Поэтому  iterable упаковка и распаковка не поддерживается, а значит
  ...
  - Похожим образом `(a, b := 16, 19)` эквивалентно `(a, (b := 16), 19)`, что является ничем иным, как трехэлементным кортежем. 
